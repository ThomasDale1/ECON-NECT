...
### ResolverCoherencia
File: apps/web/components/nect/resolver-coherencia.tsx
Tipo: card de decisión + diálogo de confirmación
Clases: contenedor `rounded-lg border bg-card p-4`; dos opciones `rounded-lg border bg-muted/40 p-3` con hover `border-primary bg-primary/5`; diálogo con la misma estructura `dl grid-cols-[auto_1fr]` que `PropagarTraslado`.
Tokens de color: borde `veredicto-en-riesgo/25` en el contenedor (solo aparece con R2); sin escala nueva.
Notas: se despliega desde "Tomar acción" de `DialogoAccion` (centro de comando) cuando la regla R2 dispara. Mantener Prisma cancela el traslado en Startrack (definitivo); mantener Startrack pasa el equipo a DISPONIBLE en Prisma. El servidor verifica rol y recurso propio; la vista se refresca al cerrar el rastro. Reemplaza a la vista `/expediente`, retirada el 13 de septiembre de 2026 por duplicar el centro de comando.
Registrado: 13 de septiembre de 2026
...
Clases: `Popover`/`PopoverTrigger`/`PopoverContent` de shadcn (base-ui) con `openOnHover` (`delay` 150 ms, `closeDelay` 150 ms); también abre con foco y clic (el disparador es un `<button>`). Contenido `w-[24rem] max-w-[calc(100vw-2rem)] max-h-(--available-height) overflow-y-auto p-0`; una sola sección extra ("Qué haría falta") separada con `border-t border-border px-4 py-3` y título en `font-label text-[10px] font-bold uppercase tracking-widest text-muted-foreground`; motivo del servidor en `rounded-lg border border-dashed border-border bg-muted`.
Tokens de color: ninguno de §1.1 — "sin asignación posible" sigue neutro (nunca rojo ni violeta). El paso del embudo donde se cae la solicitud va en `font-semibold` subrayado y los pasos siguientes en `opacity-50` con "—", así el significado no depende del color.
Notas: la envuelven la barra gris de `TimelineMaquinas` y el bloque de `VistaDia`. **Versión corta** (feedback directo: "solo la información más importante"): cabecera (clase · proyecto, estado, ventana, "Ver origen"), motivo tal cual del servidor, una línea si era una APROBADA con máquina rota, embudo en una línea (`N de la clase → N operan → N libres → N con operador`, conteos de `candidatas` del servidor) y la acción sugerida. Se quitaron la explicación de la pila, las propuestas que compiten y la lista de máquinas de la clase. No recalcula nada. Nunca muestra nombres de operador ni de conductor.
Registrado: feedback directo · 13 de septiembre de 2026 · simplificada el mismo día

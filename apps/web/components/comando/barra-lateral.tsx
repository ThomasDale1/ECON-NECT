...
/** TODO carril A — S-A1: esto sale de la salud real de cada conector. */
...
const PLATAFORMAS: EstadoPlataforma[] = [
  { nombre: 'Prisma', salud: 'ok', disponibilidad: '99.8%', detalle: 'ERP · 2.4 m de latencia' },
  { nombre: 'Startrack', salud: 'ok', disponibilidad: '100%', detalle: 'GPS · ciclo de 12 s' },
]

const PUNTO_SALUD: Record<SaludFuente, string> = {
  ok: 'bg-veredicto-coherente',
  lenta: 'bg-veredicto-atencion',
  caida: 'bg-veredicto-riesgo',
}

const TEXTO_SALUD: Record<SaludFuente, string> = {
...
export function BarraLateral() {
  return (
    <aside className="flex w-60 shrink-0 flex-col justify-between overflow-hidden bg-marina text-white">
      ...
      <div className="flex flex-col gap-4 bg-marina-honda px-5 pb-4 pt-5">
        ...
        {PLATAFORMAS.map((plataforma) => (
          <div key={plataforma.nombre} className="flex items-center gap-2">
            <span className="relative flex size-[7px] shrink-0">
              ...
              <span
                aria-hidden
                className={cn('relative inline-flex size-full rounded-full', PUNTO_SALUD[plataforma.salud])}
              />
            </span>
            <div className="flex min-w-0 flex-1 flex-col gap-px">
              <div className="flex items-center justify-between">
                <span className="font-label text-[11px] font-bold text-[#b8cdd8]">{plataforma.nombre}</span>
                <span
                  className={cn(
                    'font-mono text-[11px] font-bold',
                    TEXTO_SALUD[plataforma.salud],
                  )}
                >
                  {plataforma.disponibilidad}
                </span>
              </div>
              <span className="font-mono text-[9px] text-marina-media">{plataforma.detalle}</span>
            </div>
          </div>
        ))}
